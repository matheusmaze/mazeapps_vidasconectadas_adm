<template>
  <div class="my-6">
    <h3 class="font-bold text-primary">
      Editar {{ route.name.split("/")[1].slice(0, -1).toLowerCase() }}
    </h3>
  ...
  <div class="shadow-sm bg-white border p-8">
    ...
    <span class="lg:inline-grid w-full grid-cols-2 gap-x-4">
      <div class="inline-block w-5/6 flex-1 m-3">
      ...
      <span class="lg:inline-grid w-full grid-rows-2 lg:pb-32">
        <div class="inline-block flex-1 my-3">
          <Input
            v-model="data.status"
            type="select"
...
export default {
  ...
  setup() {
    ...
    const index = async () => {
        ...
        response['users_id'] = users.value.find(user =>  user.id === response.users_id).name;
