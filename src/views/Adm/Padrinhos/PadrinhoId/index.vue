<template>
  <div class="w-full mt-6 mb-8">
    <h3 class="font-bold text-primary">{{ route.name.split('/')[1].slice(0, -2)}} / {{userData.name}}</h3>
    <span class="text-sm text-gray-500"
      >Aqui você pode observar todos os itens relacionados ao {{route.name.split('/')[1].slice(0, -2).toLowerCase()}} {{userData.name}}, como
  ...
  <div class="card mt-10 mb-20">
    ...
    <div class="flex justify-end mt-5">
      <router-link :to="`${route.path.split('/').splice(0, 4).join('/')}/${userData.id}/criar`">
...
export default {
  ...
  setup() {
    const headers = [
      { key: "ID", value: "id"},
      { key: "Criação", value: "created_at", date: true},
    ...
    const show = (donation) => {
      if(donation.donationStatus === 'Ativa'){
        router.push(`${route.path.split('/').splice(0, 4).join('/')}/${userData.value.id}/editar/${donation.id}`);
      } else {
        alert.open("Erro", 'Doação não encontrada.', "danger");
      }
    };
