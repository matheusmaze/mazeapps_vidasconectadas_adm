<template>
  <div class="my-6">
    ...
    <span class="text-sm text-gray-500"
      >Preencha as informações abaixo e clique no botão <b>cadastrar compra</b> para
  ...
  <div class="flex justify-end mt-5">
    <Button @click="create" color="primary" :text="`Cadastrar compra`" />
  </div>
</template>
    ...
    onMounted( async () => {
      try {
        ...
        users.value.map(user => {
          usersName.value.push({label: user.name, value: user.id});
        });
