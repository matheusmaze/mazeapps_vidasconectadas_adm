<template>
  <div class="home w-full">
    <h2 class="title mx-2 py-6 text-4xl font-medium dark:text-gray-200">
      Dashboard
    </h2>
    <Alert
      ...
      @click="event"
    />
    <div class="container flex mb-6 justify-flex-end">
    ...
    <div class="card shadow-lg border rounded-lg p-8 bg-white mt-10">
      ...
      <div>
        <div class="flex-3 w-full my-8">
          <h3 class="font-bold">Aniversariantes</h3>
          <span class="text-sm text-gray-500"
...
import { retornaLogin } from "../services/util";
...
export default defineComponent({
  ...
  setup() {
    ...
    onMounted(async () => {
      ...
      try {
        users.value = await GET("user");
        const response = await GET(`dashboard/${dataAtual.toISOString()}`);
        data.value = response;
        revenue.value[0] = parseFloat(response.revenue.money);
        revenue.value[1] = parseFloat(response.revenue.card);
        ticket.value[0] = response.ticket.withdraw;
        ticket.value[1] = response.ticket.delivery;
        ...
        const ordersStatus = await GET(`orders_status`);
        const countStatus = await GET(`count_orders_by_status`);
        status.value = ordersStatus;
        status.value.forEach((stat) => {
          stat.qtd = countStatus[stat.id];
        });
        status.value[0].icon = "list";
        status.value[1].icon = "box";
        status.value[2].icon = "shopping-cart";
        status.value[3].icon = "shopping-bag";
        status.value[4].icon = "close-r";
