<template>
  <div class="my-6">
    ...
    <span class="text-sm text-gray-500"
      >Olá, está sendo criada uma doação para o <b>{{userData.name}}</b>, 
...
export default {
  ...
  setup() {
    ...
    const create = async () => {
      ...
      ((data.value.recurrence_interval === "MENSAL" || (data.value.recurrence_interval === 'UNICA' && schedulePayment.value === '1')) && data.value.recurrence_day == '0') && (data.value['recurrence_day'] = '');
      ...
      if (validate) {
        ...
        try {
          ...
          if (data.value.recurrence_interval === 'UNICA') { 
            schedulePayment.value === '1'
              ? await POST("donations", data.value)
              : await POST("donations/unique", data.value);
          } else {
            await POST("donations", data.value);
          }
          alert.open(
    ...
    onMounted(async () => {
      try {
        ...
        userData.value = await GET(`users/${info.id}`);
