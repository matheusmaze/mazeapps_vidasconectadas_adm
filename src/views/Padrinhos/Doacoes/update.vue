      ...
      const handle = async () => {
        ...
        if (validate) {
          ...
          try {
            ...
            if (data.value.recurrence_interval === 'Única') { 
            schedulePayment.value === '1'
                ? await PUT(`donations/${route.params.id}`, data.value)
                : await PUT(`donations/${route.params.id}/unique`, data.value);
            } else {
              await PUT(`donations/${route.params.id}`, data.value)
            }
            alert.open(
    ...
    onMounted(async () => {
      try {
        ...
        transactions.value.map( transaction => {
          ...
          (transaction.payment_voucher) 
          ? transaction['payment_voucher'] = 'Sim'
          : transaction['payment_voucher'] = 'Não';
        })
        loader.close();
      } catch (e) {
        loader.close();
