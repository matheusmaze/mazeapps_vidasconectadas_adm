...
export default {
  ...
  setup() {
    ...
    const defaultLabel = ref(
      route.name.split('-')[0].toLowerCase().slice(0, 6)
    );
    ...
    const create = async () => {
      ...
      if (validate || emailValidate || !documentValidate || !birthdayValidate) {
        if(validate){
          ...
          alert.open("Atenção!", "*CPF ou CNPJ inválido.", "warning");
        }  else {
        ...
        try {
          const response = await POST("users/register", data.value);
          alert.open(
            "Sucesso!",
          ...
          loader.close();
          router.push(`/${defaultLabel.value}es/detalhes/${response.user.id}`);
        } catch (e) {
          if(e){
            if(e.includes("Token")) {
